...
describe WGif::CLI do
  ...
  context 'error handling' do
    ...
    it 'catches invalid URLs' do
      WGif::ArgumentParser.any_instance.stub(:parse).and_raise(WGif::InvalidUrlException)
    ...
    it 'catches invalid timestamps' do
      WGif::ArgumentParser.any_instance.stub(:parse).and_raise(WGif::InvalidTimestampException)
      expect{ cli.make_gif([]) }.to raise_error(SystemExit)
    ...
    it 'catches missing output args' do
      WGif::ArgumentParser.any_instance.stub(:parse).and_raise(WGif::MissingOutputFileException)
      expect{ cli.make_gif([]) }.to raise_error(SystemExit)
      expect_help_with_message(@mock_stdout.string, 'Please specify an output file.')
    end

    it 'catches missing videos' do
      WGif::ArgumentParser.any_instance.stub(:parse).and_raise(WGif::VideoNotFoundException)
      expect{ cli.make_gif([]) }.to raise_error(SystemExit)
    ...
    it 'catches encoding exceptions' do
      WGif::ArgumentParser.any_instance.stub(:parse).and_raise(WGif::ClipEncodingException)
      expect{ cli.make_gif([]) }.to raise_error(SystemExit)
      expect_help_with_message(@mock_stdout.string, "WGif encountered an error transcoding the video.")
    end

    it 'catches upload errors' do
      WGif::ArgumentParser.any_instance.stub(:parse).and_raise(WGif::ImgurException, "Imgur error")
      expect{ cli.make_gif([]) }.to raise_error(SystemExit)
